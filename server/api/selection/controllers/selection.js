...
module.exports = {
	...
	async create(ctx) {
		...
		if (ctx.is('multipart')) return ctx.badRequest(
			...
			{ id: 'Selection.create.format.invalid', error: 'ValidationError' }
		)
		...
		if (!strapi.services.validator.isInt(classroom) || !strapi.services.validator.isInt(learning_standard)) return ctx.badRequest(
			'A classroom and learning standard must be provided!',
			{ id: 'Selection.create.body.invalid', error: 'ValidationError' }
		)

		// ensure the learning standard is valid
		const validStandard = await strapi.services['learning-standard'].findOne({ id: learning_standard })
		if (validStandard === null) return ctx.notFound(
			'The learning standard provided is invalid!',
			{ id: 'Selection.create.learning_standard.invalid', error: 'ValidationError' }
		)

		// try to disable the current selection for the classroom
		try {
			const entry = await strapi.query('selection').update({ classroom, current: true }, { current: false });
		} catch (err) {
		...
		const selection = await strapi.services.selection.create({ classroom, learning_standard })
