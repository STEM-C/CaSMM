    ...
    async me(ctx) {
      ...
      return {
        ...
        students: students.map(student =>
          sanitizeEntity(student, { model: strapi.models.student })
        ),
      }
    },
    /**
    ...
    async enrolled(ctx) {
      ...
      if (ctx.is("multipart"))
        return ctx.badRequest("Multipart requests are not accepted!", {
          id: "Student.enrolled.format.invalid",
      ...
      if (params !== 1)
        return ctx.badRequest("Invalid number of params!", {
          id: "Student.enrolled.body.invalid",
          error: "ValidationError",
        })
      ...
      if (typeof enrolled !== "boolean")
        return ctx.badRequest("An enrollment status must be provided!", {
          id: "Student.enrolled.body.invalid",
          error: "ValidationError",
        })
      ...
      const updatedStudent = await strapi.services.student.update(
        { id: id },
        student
      )
        ...
        return await Promise.all(
          students.map(student => {
            ...
            if (typeof student.name !== "string" || !student.name)
              return ctx.badRequest("A name must be provided!", {
                id: "Student.create.body.invalid",
                error: "ValidationError",
              })
