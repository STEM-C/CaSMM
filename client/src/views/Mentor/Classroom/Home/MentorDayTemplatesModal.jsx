...
import DayComponentTags from "../../../ContentCreator/DayEditor/components/DayComponentTags"

const MentorDayTemplates = ({ thisDay, viewing, setViewing, learningStandard  }) => {
  ...
  const handleViewDayTemplate = async (day, index) => {
    if (localStorage.sandbox == true){
      handleViewDay(day, learningStandard.name)
    }
    else {
                ...
                renderItem={(item, index) => (
                  <List.Item>
                    <Card
                      ...
                      onClick={() => handleViewDayTemplate(days[0], index, false)}
                    />
                    <span
...
export default MentorDayTemplates
