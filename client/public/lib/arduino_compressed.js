...
Blockly.Arduino.init = function (a) {
  ...
  for (var c in b)
    Blockly.Arduino.addVariable(
      c,
      Blockly.Arduino.getArduinoType_(b[c]) +
        ' ' +
        Blockly.Arduino.variableDB_.getName(c, Blockly.Variables.NAME_TYPE) +
        ';'
    );
};
Blockly.Arduino.finish = function (a) {
  ...
  for (f in Blockly.Arduino.definitions_)
    c.push(Blockly.Arduino.definitions_[f]);
  c.length && c.push('\n');
  for (f in Blockly.Arduino.codeFunctions_)
    e.push(Blockly.Arduino.codeFunctions_[f]);
  for (f in Blockly.Arduino.userFunctions_)
    e.push(Blockly.Arduino.userFunctions_[f]);
  e.length && e.push('\n');
  var g = [''],
    h = '';
  void 0 !== Blockly.Arduino.setups_.userSetupCode &&
...
Blockly.Arduino.reservePin = function (a, b, c, d) {
  void 0 !== Blockly.Arduino.pins_[b]
    ? Blockly.Arduino.pins_[b] != c
      ? a.setWarningText(
          Blockly.Msg.ARD_PIN_WARN1.replace('%1', b)
            .replace('%2', d)
            .replace('%3', c)
            .replace('%4', Blockly.Arduino.pins_[b]),
          d
        )
      : a.setWarningText(null, d)
    : ((Blockly.Arduino.pins_[b] = c), a.setWarningText(null, d));
};
  ...
  if (!a.outputConnection || !a.outputConnection.targetConnection) {
    ...
    for (var e = 0; e < a.inputList.length; e++)
      a.inputList[e].type == Blockly.INPUT_VALUE &&
        ...
        (d = this.allNestedComments(d)) &&
        (c += this.prefixLines(d, '// '));
  }
...
Blockly.Arduino.spi_transfer = function (a) {
  ...
  for (
    var e = Blockly.Arduino.Boards.selected.spiPins[b], f = 0;
    f < e.length;
    f++
  )
  ...
  'none' !== c &&
    ...
    Blockly.Arduino.addSetup('io_' + c, 'pinMode(' + c + ', OUTPUT);', !1));
  a = [];
...
Blockly.Arduino.procedures_defreturn = function (a) {
  var b = Blockly.Arduino.variableDB_.getName(
      a.getFieldValue('NAME'),
      Blockly.Procedures.NAME_TYPE
    ),
    c = Blockly.Arduino.statementToCode(a, 'STACK');
  Blockly.Arduino.STATEMENT_PREFIX &&
    (c =
      Blockly.Arduino.prefixLines(
        Blockly.Arduino.STATEMENT_PREFIX.replace(/%1/g, "'" + a.id + "'"),
        Blockly.Arduino.INDENT
      ) + c);
  Blockly.Arduino.INFINITE_LOOP_TRAP &&
    (c =
      Blockly.Arduino.INFINITE_LOOP_TRAP.replace(/%1/g, "'" + a.id + "'") + c);
  var d =
    Blockly.Arduino.valueToCode(a, 'RETURN', Blockly.Arduino.ORDER_NONE) || '';
  d && (d = '  return ' + d + ';\n');
  for (var e = [], f = 0; f < a.arguments_.length; f++)
    e[f] =
      Blockly.Arduino.getArduinoType_(a.getArgType(a.arguments_[f])) +
      ' ' +
...
Blockly.Arduino.procedures_callreturn = function (a) {
  for (
    ...
    c[d] =
      Blockly.Arduino.valueToCode(a, 'ARG' + d, Blockly.Arduino.ORDER_NONE) ||
      'null';
...
Blockly.Arduino.procedures_ifreturn = function (a) {
  var b =
    'if (' +
    (Blockly.Arduino.valueToCode(a, 'CONDITION', Blockly.Arduino.ORDER_NONE) ||
      'false') +
    ') {\n';
  a.hasReturnValue_
    ? ((a =
        Blockly.Arduino.valueToCode(a, 'VALUE', Blockly.Arduino.ORDER_NONE) ||
        'null'),
      (b += '  return ' + a + ';\n'))
    : (b += '  return;\n');
  return b + '}\n';
...
Blockly.Arduino.servo_write = function (a) {
  ...
  Blockly.Arduino.reservePin(
    a,
    b,
    Blockly.Arduino.PinTypes.SERVO,
    'Servo Write'
  );
...
Blockly.Arduino.Boards.generateAnalogIo = function (a, b) {
  for (var c = [], d = a; d < b + 1; d++)
    c.push(['A' + d.toString(), 'A' + d.toString()]);
  return c;
};
...
Blockly.Arduino.Boards.changeBoard = function (a, b) {
  if (void 0 === Blockly.Arduino.Boards.profiles[b])
    console.log('Tried to set non-existing Arduino board: ' + b);
  else {
...
Blockly.Arduino.Boards.refreshBlockFieldDropdown = function (a, b, c) {
  ...
  d.menuGenerator_ = c;
  d = !1;
  for (var e = 0; e < c.length; e++) b == c[e][1] && (d = !0);
  d
    ? a.setWarningText(null, 'bPin')
    : a.setWarningText(
        'The old pin value ' + b + ' is no longer available.',
        'bPin'
      );
};
...
Blockly.Arduino.text_join = function (a) {
  ...
  for (var c = 0; c < a.itemCount_; c++) {
    var d = Blockly.Arduino.valueToCode(
      a,
      'ADD' + c,
      Blockly.Arduino.ORDER_NONE
    );
    b[c] = '' == d ? 'String("")' : 'String(' + d + ')';
  }
  b = b.join(' + ');
...
Blockly.Arduino.text_prompt_ext = function (a) {
  ...
  e
    ? d.push('float ' + Blockly.Arduino.DEF_FUNC_NAME + '(String msg) {')
    : d.push('String ' + Blockly.Arduino.DEF_FUNC_NAME + '(String msg) {');
  ...
  e
    ? (d.push('      content = ' + b + '.parseFloat();'),
      d.push('      stringComplete = true;'))
    : (d.push('      char readChar = (char)' + b + '.read();'),
      d.push("      if (readChar == '\\n' || readChar == '\\r') {"),
      d.push('        stringComplete = true;'),
      d.push('      } else {'),
      d.push('        content += readChar;'),
      d.push('      }'));
  d.push('    }');
  d.push('  }');
  ...
  d.push('}');
  c = Blockly.Arduino.addFunction('getUserInputPrompt' + c, d.join('\n'));
  Blockly.Arduino.addSetup('serial_' + b, b + '.begin(9600);', !1);
  a =
    Blockly.Arduino.valueToCode(a, 'TEXT', Blockly.Arduino.ORDER_NONE) || '""';
...
Blockly.Arduino.io_digitalwrite = function (a) {
  var b = a.getFieldValue('PIN'),
    c =
      Blockly.Arduino.valueToCode(a, 'STATE', Blockly.Arduino.ORDER_ATOMIC) ||
      'LOW';
  Blockly.Arduino.reservePin(
    a,
    b,
...
Blockly.Arduino.io_analogwrite = function (a) {
  var b = a.getFieldValue('PIN'),
    c =
      Blockly.Arduino.valueToCode(a, 'NUM', Blockly.Arduino.ORDER_ATOMIC) ||
      '0';
  Blockly.Arduino.reservePin(
    a,
    b,
  ...
  0 > c || 255 < c
    ...
    : a.setWarningText(null, 'pwm_value');
...
Blockly.Arduino.math_arithmetic = function (a) {
  ...
  return c
    ? [d + c + a, b]
    : ['Math.pow(' + d + ', ' + a + ')', Blockly.Arduino.ORDER_UNARY_POSTFIX];
};
Blockly.Arduino.math_single = function (a) {
  ...
  if ('NEG' == b)
    return (
      ...
      '-' == a[0] && (a = ' ' + a),
      ['-' + a, Blockly.Arduino.ORDER_UNARY_PREFIX]
    );
  a =
    'ABS' == b || 'ROUND' == b.substring(0, 5)
      ...
      : Blockly.Arduino.valueToCode(a, 'NUM', Blockly.Arduino.ORDER_NONE) ||
        '0';
  switch (b) {
    case 'ABS':
      var c = 'abs(' + a + ')';
...
Blockly.Arduino.math_change = function (a) {
  var b =
    Blockly.Arduino.valueToCode(a, 'DELTA', Blockly.Arduino.ORDER_ADDITIVE) ||
    '0';
  return (
    Blockly.Arduino.variableDB_.getName(
      a.getFieldValue('VAR'),
...
Blockly.Arduino.math_constrain = function (a) {
  var b =
      ...
      '0',
    c =
      Blockly.Arduino.valueToCode(a, 'LOW', Blockly.Arduino.ORDER_NONE) || '0';
...
Blockly.Arduino.math_random_int = function (a) {
  var b =
    Blockly.Arduino.valueToCode(a, 'FROM', Blockly.Arduino.ORDER_NONE) || '0';
...
Blockly.Arduino.controls_if = function (a) {
  var b = 0,
    c =
      Blockly.Arduino.valueToCode(a, 'IF' + b, Blockly.Arduino.ORDER_NONE) ||
      'false',
    d = Blockly.Arduino.statementToCode(a, 'DO' + b),
    e = 'if (' + c + ') {\n' + d + '}';
  for (b = 1; b <= a.elseifCount_; b++)
    (c =
      ...
      (e += ' else if (' + c + ') {\n' + d + '}');
  a.elseCount_ &&
    ((d = Blockly.Arduino.statementToCode(a, 'ELSE')),
    (e += ' else {\n' + d + '}'));
  return e + '\n';
};
...
Blockly.Arduino.logic_ternary = function (a) {
  var b =
      Blockly.Arduino.valueToCode(a, 'IF', Blockly.Arduino.ORDER_CONDITIONAL) ||
      'false',
    c =
      Blockly.Arduino.valueToCode(
        a,
        'THEN',
        Blockly.Arduino.ORDER_CONDITIONAL
      ) || 'NULL';
  a =
    Blockly.Arduino.valueToCode(a, 'ELSE', Blockly.Arduino.ORDER_CONDITIONAL) ||
    'NULL';
  return [b + ' ? ' + c + ' : ' + a, Blockly.Arduino.ORDER_CONDITIONAL];
};
Blockly.Arduino.comment = {};
...
Blockly.Arduino.variables_set_type = function (a) {
  ...
  return [
    '(' +
      Blockly.Arduino.getArduinoType_(
        Blockly.Types[a.getFieldValue('VARIABLE_SETTYPE_TYPE')]
      ) +
      ')(' +
      b +
      ')',
...
Blockly.Arduino.serial_print = function (a) {
  for (
    var b = a.getFieldValue('SERIAL_ID'),
      ...
      d = 'TRUE' == a.getFieldValue('NEW_LINE'),
      e = Blockly.Arduino.Boards.selected.serialPins[b],
      f = 0;
    f < e.length;
    f++
  )
    Blockly.Arduino.reservePin(
      a,
      e[f][1],
      Blockly.Arduino.PinTypes.SERIAL,
      'SERIAL ' + e[f][0]
    );
...
Blockly.Arduino.stepper_config = function (a) {
  ...
  'FOUR' === d &&
    (g.push(a.getFieldValue('STEPPER_PIN3')),
    g.push(a.getFieldValue('STEPPER_PIN4')));
  d = 'int ' + c + '[' + g.length + '] = {';
  e = 'Stepper stepper_' + c + '(' + e + ', ';
  for (var h = 0; h < g.length; h++)
    Blockly.Arduino.reservePin(a, g[h], b, 'Stepper'),
      (d += g[h] + ', '),
      (e += g[h] + ', ');
  d = d.slice(0, -2) + '};';
  e = e.slice(0, -2) + ');';
...
Blockly.Arduino.controls_repeat = function (a) {
  ...
  a = Blockly.Arduino.variableDB_.getDistinctName(
    'count',
    Blockly.Variables.NAME_TYPE
  );
  return (
...
Blockly.Arduino.controls_repeat_ext = function (a) {
  var b =
      Blockly.Arduino.valueToCode(a, 'TIMES', Blockly.Arduino.ORDER_ADDITIVE) ||
      '0',
    c = Blockly.Arduino.statementToCode(a, 'DO');
  c = Blockly.Arduino.addLoopTrap(c, a.id);
  a = '';
  var d = Blockly.Arduino.variableDB_.getDistinctName(
      'count',
      Blockly.Variables.NAME_TYPE
    ),
    e = b;
  ...
  var b = 'UNTIL' == a.getFieldValue('MODE'),
    c =
      Blockly.Arduino.valueToCode(
        a,
        'BOOL',
        b ? Blockly.Arduino.ORDER_LOGICAL_OR : Blockly.Arduino.ORDER_NONE
      ) || 'false',
...
Blockly.Arduino.controls_for = function (a) {
  var b = Blockly.Arduino.variableDB_.getName(
    ...
    d =
      Blockly.Arduino.valueToCode(a, 'TO', Blockly.Arduino.ORDER_ASSIGNMENT) ||
      '0',
    e =
      Blockly.Arduino.valueToCode(a, 'BY', Blockly.Arduino.ORDER_ASSIGNMENT) ||
      '1',
  ...
  if (Blockly.isNumber(c) && Blockly.isNumber(d) && Blockly.isNumber(e)) {
    ...
    (a = ''),
      ...
      c.match(/^\w+$/) ||
        Blockly.isNumber(c) ||
        ((g = Blockly.Arduino.variableDB_.getDistinctName(
          b + '_start',
          Blockly.Variables.NAME_TYPE
        )),
        (a += 'int ' + g + ' = ' + c + ';\n')),
      ...
      d.match(/^\w+$/) ||
        Blockly.isNumber(d) ||
        ((c = Blockly.Arduino.variableDB_.getDistinctName(
          b + '_end',
          Blockly.Variables.NAME_TYPE
        )),
        (a += 'int ' + c + ' = ' + d + ';\n')),
      (d = Blockly.Arduino.variableDB_.getDistinctName(
        b + '_inc',
        Blockly.Variables.NAME_TYPE
      )),
      (a += 'int ' + d + ' = '),
...
Blockly.Arduino.base_map = function (a) {
  var b =
    Blockly.Arduino.valueToCode(a, 'NUM', Blockly.Arduino.ORDER_NONE) || '0';
  a =
    Blockly.Arduino.valueToCode(a, 'DMAX', Blockly.Arduino.ORDER_ATOMIC) || '0';
